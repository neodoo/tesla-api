...
public class StopHvacSystem {
	
	private final static Logger log = Logger.getLogger(StopHvacSystem.class.getName());

	public static final String URL_STOP_HVAC_SYSTEM ="/command/auto_conditioning_stop";

	private TeslaInvoker teslaInvoker;

	public StopHvacSystem(TeslaInvoker teslaInvoker) {
	...
	public StopHvacSystemResponse execute(int vehicleId) throws OauthInvokerException, TeslaInvokerException{
		...
		try {
			...
			WebResource webResource = client.resource(teslaInvoker.getUri() + "/" + TeslaInvoker.URL_PATH_VEHICLES + "/" + vehicleId + URL_STOP_HVAC_SYSTEM);
			...
			stopHvacSystemResponse =  StopHvacSystemResponse.toObject(output);
			...
			throw new TeslaInvokerException("Error invoking stop HVAC system service: " + e.getMessage());
		}
